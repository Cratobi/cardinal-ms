...
class Draft extends Component {
	...
	handlePublishOrder = () => {
		...
		for (let rowindex = 0; rowindex < tabledata.table_date.length; rowindex++) {
			tabledata.table_date[rowindex][1] = new Date(tabledata.table_date[rowindex][1]).getTime()
		}

		console.log(tabledata)
		this.props.publishOrder(this.props.metadata.get('id'), tabledata, this.props.history)
	}
	...
	render() {
		return this.props.metadata ? (
			<MonoGridLayout
				...
				header={
					<Fragment>
						<div className="draft-header-btns p-r-1">
							<button onClick={() => this.handleConfirmationModalDelete(true)} className="btn btn-caution p-l-1 p-r-1">
							...
							>
								<Modal
									...
									footer={
										<Fragment>
											<div onClick={() => this.handleConfirmationModalDelete(false)} className="btn btn-light">
												Cancel
											</div>
											<div
												onClick={() => {
													this.props.deleteDraft(this.props.metadata.get('id'), this.props.history)
							...
							<button className="btn btn-dark p-l-1 p-r-1" onClick={this.handleSendDraftTabledata}>
							...
							<div className="draft-header">
								<table className="overview">
									<tbody>
										<tr>
											<td>Shipment Date:</td>
											<td>{new Date(this.props.metadata.get('shipment_date')).toDateString()}</td>
										</tr>
										<tr>
											<td>Buyer:</td>
				...
				footer={
					<Fragment>
						<button className="btn btn-success f-r" onClick={() => this.handleConfirmationModalPublish(true)}>
							<i className="fas fa-check p-r" />
							Publish Order
						</button>

						<CSSTransition in={this.state.confirmationModalPublish} timeout={250} classNames="anim-modal" unmountOnExit>
							<Modal
								...
								footer={
									<Fragment>
										<div onClick={() => this.handleConfirmationModalPublish(false)} className="btn btn-light">
											Cancel
										</div>
										<div onClick={this.handlePublishOrder} className="btn btn-success">
											Confirm Publish
										</div>
									</Fragment>
								}
						...
						<button className="btn btn-lighter f-r" onClick={this.handleSendDraftTabledata}>
							<i className="fas fa-inbox p-r" />
							Save to Draft
						</button>
					</Fragment>
			...
			>
				<CSSTransition in={this.props.CnP_data ? true : false} timeout={250} classNames="slide-up" unmountOnExit>
					<EditableTable editability={true} schema={this.props.CnP_dataSchema} data={this.props.CnP_data} />
				</CSSTransition>
			</MonoGridLayout>
		) : (
...
const mapDispatchToProps = dispatch => {
	return {
		...
		sendDraftTabledata: payload => dispatch(actions.sendDraftTabledata(payload)),
		onChange: payload => dispatch(actions.onChange(payload)),
		publishOrder: (id, tabledata, router) => dispatch(actions.publishOrder(id, tabledata, router)),
		deleteDraft: (id, router) => dispatch(actions.deleteDraft(id, router)),
		resetDraft: () => dispatch(actions.resetDraft())
	}
}

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(Draft))
