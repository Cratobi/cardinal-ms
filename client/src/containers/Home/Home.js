...
class Home extends Component {
	...
	sendDraftMetadata = e => {
		...
		if (this.state.formFilled) {
			...
			draft.tabledata = this.props.data.toJS()
			...
			<div className="container home-container">
				<div className="home-mm">
					<div className="home-mm-content">
						...
						<CSSTransition in={this.state.searchFocus} timeout={250} classNames="fade" unmountOnExit>
									...
									{this.props.search_result && this.state.searchQuery !== '' ? (
										<div>
											{this.props.search_result.map((order, index) => (
												...
												>
													{order.get('order_no')}
													<span className="txt-small">{order.get('style_no')}</span>
												</Link>
											))}
										...
										<div className="help-txt">Search order or add @ to search style</div>
									) : (
				...
				<div className="home-cards-header">
					...
					<span className="d-flex">
						...
						<button className="btn btn-chip btn-success" onClick={() => this.handleOrderModal(true)}>
				...
				<div className="home-cards-container order-cards-small">
					{this.props.orders ? <OrderCards orders={this.props.orders} /> : <CardLoadingLayout orders={20} />}
				</div>

				{/* MODAL */}

				<CSSTransition in={this.state.draftModal} timeout={250} classNames="anim-modal" unmountOnExit>
					<ModalLayout
						key="1"
						tittle="ADD ORDER"
						...
						footer={
							<div
								...
								className={`btn btn-success${!this.state.formFilled ? ' btn-success-disabled' : ''}`}
							>
								Create Order
							</div>
						}
...
const mapStateToProps = state => {
	return {
		...
		data: state.getIn(['draft', 'CnP_data']),
...
const mapDispatchToProps = dispatch => {
	return {
		...
		sendDraftMetadata: (payload, router) => dispatch(actions.sendDraftMetadta(payload, router)),
	}
}
