		...
		<Fragment>
			<header>
				<Fragment>
					{/* Logo */}
					<span className='logo'>
						<CSSTransition in={props.backBtn} timeout={250} classNames='slide-right' unmountOnExit>
							<Link to='/' style={{ position: 'absolute' }} className='btn btn-caution back-btn'>
            ...
            />
					...
					<span className='nav-btn'>
						<span
							onClick={() => props.handleMenuNotification(!props.notificationMenu)}
							className={`btn btn-custom-notification ${props.notificationUnread !== 0
								? 'btn-caution'
						...
						<span
							...
							className={`btn btn-custom-account m-r ${props.accountMenu ? 'btn-custom-account-active' : ''}`}
						>
							<span className='btn-custom-account-btn'>
								<i className='fas fa-user' />
								<span className='p-l p-r hide-s'>{props.userInfo.get('username')}</span>
							</span>
							<CSSTransition in={props.accountMenu} timeout={250} classNames='rotate-180'>
			...
			>
				<div className='menu-search'>
					<span className='backdrop backdrop-transparent backdrop-menu' onClick={props.handleSearchFocus} />
					<div className='scrollable'>
						{props.searchResult ? (
							props.searchResult.map((order, index) => (
								...
								>
									{order.get('order_no')}
									<span className='txt-emp txt-small'>{order.get('style_no')}</span>
								</Link>
							))
						) : (
							<LoadingLayout />
			...
			{props.accountMenu || props.notificationMenu ? (
				<div className='backdrop backdrop-transparent backdrop-menu' onClick={() => props.handleMenuAccount(false)} />
			) : null}
			<CSSTransition in={props.accountMenu} timeout={140} classNames='anim-menu' unmountOnExit>
				<div className='menu menu-account'>
					...
					<div className='footer'>
						{props.userInfo.get('power') === 'admin' ? (
							<button className='btn btn-transparent' onClick={props.handleDashboardLink}>
								<i className='fas fa-tachometer-alt' />
								DASHBOARD
							</button>
						) : null}
						<button className='btn btn-transparent' onClick={() => props.handleModalSettings(true)}>
			...
			<CSSTransition in={props.notificationMenu} timeout={140} classNames='anim-menu' unmountOnExit>
