...
const setExtraData = (state, rowindex, colindex, value) => {
  value = emptifier(value)
  state = !colindex
    ? state.setIn(['CnP_data', 'table_extradata', rowindex], value)
    : state.setIn(['CnP_data', 'table_extradata', rowindex, colindex], value)

  return state
}
const getR22 = state => state.getIn(['CnP_data', 'table_extradata', 7, 0])

const setR22 = (state, value) => {
  const SFBM = !value ? getExtraData(state, 3, 1) : value
  const compositionTotal = getComposition__Total(state)
  let R22 = compositionTotal / 12 * SFBM / 1000

  state = state.setIn(['CnP_data', 'table_extradata', 7, 0], R22)
...
const getS22 = state => state.getIn(['CnP_data', 'table_extradata', 7, 1])
...
const setS22 = state => {
  ...
  state = state.setIn(['CnP_data', 'table_extradata', 7, 1], S22)
...
const getBasic = (state, colindex) => state.getIn(['CnP_data', 'table_extradata', 8, colindex])
...
const setBasic = (state, colindex) => {
  ...
  const fabric_weight = getExtraData(state, 'table_extradata', 2)
  const s_allowance_chest = getExtraData(state, 'table_extradata', 5, 0)
  const s_allowance_length = getExtraData(state, 'table_extradata', 5, 1)
  ...
  state = state.setIn(['CnP_data', 'table_extradata', 8, colindex], basic)
