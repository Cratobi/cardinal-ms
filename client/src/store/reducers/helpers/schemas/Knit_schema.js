const KnitSchema = {
	table_items               : {
		tablename   : 'items',
		tableheader : null,
		tablebody   : [
			[
				{ rowspan: '2', cellType: 'text', cellData: 'Item/Description ' },
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Colour'
				},
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Fabrics Composition'
				},
				{
					colspan  : '10',
					cellType : 'text',
					cellData : 'Size'
				},
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : ''
				},
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Total Fabrics'
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],

			[
				{
					rowspan  : '8',
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],

			[
				{
					colspan  : '3',
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			]
		]
	},
	table_fabricfinishprocess : {
		tablename   : 'fabricfinishprocess',
		tableheader : null,
		tablebody   : [
			[
				{
					id       : 'ffp_a_1',
					editable : true,
					cellType : 'select',
					cellData : ''
				},
				{
					id       : 'ffp_a_2',
					editable : true,
					cellType : 'checkbox',
					cellData : ''
				}
			],
			[
				{
					id       : 'ffp_b_1',
					editable : true,
					cellType : 'select',
					cellData : ''
				},
				{
					id       : 'ffp_b_2',
					editable : true,
					cellType : 'checkbox',
					cellData : ''
				}
			],
			[
				{
					id       : 'ffp_c_1',
					editable : true,
					cellType : 'select',
					cellData : ''
				},
				{
					id       : 'ffp_c_2',
					editable : true,
					cellType : 'checkbox',
					cellData : ''
				}
			],
			[
				{
					id       : 'ffp_d_1',
					editable : true,
					cellType : 'select',
					cellData : ''
				},
				{
					id       : 'ffp_d_2',
					editable : true,
					cellType : 'checkbox',
					cellData : ''
				}
			],
			[
				{
					id       : 'ffp_e_1',
					editable : true,
					cellType : 'select',
					cellData : ''
				},
				{ id: 'ffp_e_2', editable: true, cellType: 'checkbox', cellData: '' }
			]
		]
	},
	table_quantity            : {
		tablename   : 'fabricfinishprocess',
		tableheader : null,
		tablebody   : [
			[
				{
					cellType : 'text',
					cellData : 'Yarn Qty'
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Elastane Qty'
				},
				{
					cellType : 'number',
					cellData : ''
				}
			]
		]
	},
	table_extrafabric         : {
		tablename   : 'extrafabric',
		tableheader : [
			{ cellData: 'Contrast Color' },
			{ cellData: 'Construction' },
			{ cellData: 'Composition' },
			{ cellData: 'Fabric Weight' },
			{ cellData: 'Qty' }
		],
		tablebody   : [
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					colspan  : 4,
					cellType : 'text',
					cellData : 'G. Total'
				},
				{
					cellType : 'number',
					cellData : ''
				}
			]
		]
	},
	table_rawitems            : {
		tablename   : 'items',
		tableheader : null,
		tablebody   : [
			[
				{ rowspan: '2', cellType: 'text', cellData: 'Item/Description ' },
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Colour'
				},
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Fabrics Composition'
				},
				{
					colspan  : '10',
					cellType : 'text',
					cellData : 'Size'
				},
				{
					rowspan  : '2',
					cellType : 'text',
					cellData : 'Total Fabrics'
				}
			],
			[
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],

			[
				{
					rowspan  : '8',
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			],
			[
				{
					colspan  : '3',
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				}
			]
		]
	},
	table_fabricsl            : {
		tablename   : 'fabricsl',
		tableheader : [
			{ cellData: 'Color' },
			{ cellData: 'Fabrics Type' },
			{ cellData: 'M/C Dia' },
			{ cellData: 'S/L' },
			{ cellData: 'Qty' },
			{ cellData: 'Remarks' }
		],
		tablebody   : [
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				},
				{
					cellType : 'number',
					cellData : ''
				},
				{
					cellType : 'text',
					cellData : ''
				}
			]
		]
	},

	table_date                : {
		tablename   : 'date',
		tableheader : null,
		tablebody   : [
			[
				{
					cellType : 'text',
					cellData : 'Yarn Required Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Knitting Start Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Knitting Close Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Batch Received Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Batch Delivery Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Dyeing Start Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Dyeing Close Date'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Finished Fabrics Check Close'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			],
			[
				{
					cellType : 'text',
					cellData : 'Finished Fabrics Inhouse'
				},
				{
					cellType : 'date',
					cellData : ''
				}
			]
		]
	}
}

export default KnitSchema
