	...
	switch (action.type) {
		...
		case 'SYNCTABLES': {
			...
			if (tablename === 'extradata') {
				...
				if (rowindex === 3 && colindex === 1) state = syncHandlers.setR22(state, value)
				if (rowindex === 2 || rowindex === 4 || rowindex === 5)
					syncHandlers.loop(0, 9, (colindex) => (state = syncHandlers.setBasic(state, colindex)))

				return state
			}
			if (tablename === 'measurments') {
				state = syncHandlers.setCellData(state, tablename, rowindex, colindex, value)
				if (rowindex === 0 && colindex !== 0) state = syncHandlers.composition__CloneSizeHeader(state, colindex, value)
			...
			if (tablename === 'accessoriesname' && 0 <= rowindex && rowindex <= 21 && colindex === 2) {
				const prev_val = syncHandlers.getCellData(state, tablename, rowindex, colindex)
				state = syncHandlers.setCellData(state, tablename, rowindex, colindex, value)
			...
			if (tablename === 'currency') {
				...
				if (rowindex === 0)
					syncHandlers.loopFor([ 1, 2, 3, 5, 7, 8 ], (rowindex) => (state = syncHandlers.price__Value(state, rowindex)))
